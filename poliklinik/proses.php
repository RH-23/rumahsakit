...
if(isset($_POST['add'])) {
	...
	for ($i=1; $i <=$total ; $i++) {
		...
		$sql = mysqli_query($con, "INSERT INTO tb_poliklinik (id_poli, nama_poli, gedung) VALUES('$uuid', '$nama', '$gedung')") or die (mysql_error($con));
	}
	if ($sql) {
	...
	for ($i=0; $i<count($_POST['id']); $i++) {
		...
		$sql = mysqli_query($con, "UPDATE tb_poliklinik SET nama_poli='$nama', gedung='$gedung' WHERE id_poli = '$id'") or die (mysql_error($con));
	}
